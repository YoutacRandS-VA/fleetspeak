...
type Collector interface {
	// ConfigSynced is called when the client config is synced.
	ConfigSynced(status error)
}

// NoopCollector implements Collector by doing nothing.
type NoopCollector struct{}

// ConfigSynced does nothing.
func (c NoopCollector) ConfigSynced(status error) {}
